                                        ...
                                        Point_t& scaledStart)
{
  ...
  for (iy = 0; iy < nRows; iy = iy + nodeSize)
  {
    ...
    for (ix = 0; ix < nCols; ix = ix + nodeSize)
    {
      ...
      for (nodeRow = 0; (nodeRow < robotNodeSize) && ((iy + nodeRow - ceil(static_cast<float>(robotNodeSize - nodeSize) / 2.0)) < nRows) && (nodeOccupied == false); ++nodeRow)
      {
        for (nodeColl = 0; (nodeColl < robotNodeSize) && ((ix + nodeColl- ceil(static_cast<float>(robotNodeSize - nodeSize) / 2.0)) < nCols); ++nodeColl)
        {
          ...
          // if((ix + nodeColl - ceil(static_cast<float>(robotNodeSize - nodeSize) / 2.0))>maxcol)
          //     maxcol = (ix + nodeColl - ceil(static_cast<float>(robotNodeSize - nodeSize) / 2.0));
